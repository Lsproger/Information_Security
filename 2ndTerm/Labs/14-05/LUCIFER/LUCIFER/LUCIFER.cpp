...
static const unsigned char m2[8] = {
	0x7F, 0xBF, 0xDF, 0xEF, 0xF7, 0xFD, 0xFE
};

#define SHIFTLLEFT(x,n) (x)<<(n)
	...
	for (int round = 0; round < round_count; ++round) {
		...
		for (int step = 0; step < step_count; ++step) {
			...
			if (key[transform_countrol_byte_idx] & m1[step_count - step - 1])
				message_byte = SHIFTLLEFT(s1[HIGHSUBBYTE(message_byte)], 4) | s0[LOWSUBBYTE(message_byte)];
			else
				message_byte = SHIFTLLEFT(s0[HIGHSUBBYTE(message_byte)], 4) | s1[LOWSUBBYTE(message_byte)];
			...
			//permutation
			message_byte = 
				SHIFTRIGHT(message_byte & m1[0], 3) |
				SHIFTRIGHT(message_byte & m1[1], 4) |
				SHIFTLLEFT(message_byte & m1[2], 2) |
				SHIFTRIGHT(message_byte & m1[3], 1) |
				SHIFTLLEFT(message_byte & m1[4], 2) |
				SHIFTLLEFT(message_byte & m1[5], 4) |
				SHIFTRIGHT(message_byte & m1[6], 1) |
				SHIFTRIGHT(message_byte & m1[7], 1);
			//diffusion
