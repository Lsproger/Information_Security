...
namespace IterativeCode
{
    ...
    {
        public bool[] Xn, Xk, tmpXk, Xr, Yn, cYn, Yk, Yr, _Yr, S, E, R;
        public int n, k, r, rc, rr;
        public bool[][] H;

        public IterativeCode(bool isX, string s)
        {
            EnterWord(s);
            if(isX) CalcRKN();
        }
        ...
        public void EnterWord(string s) //enterin`g xk
        {
            int[] arr = s
                .ToCharArray().Select(x => x - '0').ToArray();
            tmpXk = arr.Select(i => Convert.ToBoolean(i)).ToArray();
        }

        public void EnterWord(bool[] s) //enterin`g Yn
        {
            Yn = s;
        }
